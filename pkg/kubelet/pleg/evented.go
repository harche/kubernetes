...
func (e *EventedPLEG) watchEventsChannel() {
	...
	containerEventsResponseCh := make(chan *runtimeapi.ContainerEventResponse, len(e.eventChannel))
	...
	for event := range containerEventsResponseCh {
		...
		case runtimeapi.ContainerEventType_CONTAINER_STOPPED_EVENT:
			...
			klog.V(3).InfoS("Recieved Container Stopped Event", "CRI container event", event)
		case runtimeapi.ContainerEventType_CONTAINER_CREATED_EVENT:
			e.updatePodStatus(event)
			klog.V(3).InfoS("Recieved Container Created Event", "CRI container event", event)
		case runtimeapi.ContainerEventType_CONTAINER_STARTED_EVENT:
			...
			klog.V(3).InfoS("Recieved Container Started Event", "CRI container event", event)
		case runtimeapi.ContainerEventType_CONTAINER_DELETED_EVENT:
			...
			klog.V(3).InfoS("Recieved Container Deleted Event", "CRI container event", event)
		}
	}
}
