...
func (e *EventedPLEG) Start() {
	...
	if !IsEventedPLEGInUse {
		...
		go wait.Forever(e.sillyCacheUpdate, 1*time.Second)
		go wait.Until(e.watchEventsChannel, 0, e.stopCh)
	}
}

// Just like Generic PLEG, we keep updating the pod cache
// timestamp every second. Without out this dummy update
// right now we end up missing the `Running` state of the pod.
// Needs more debugging.
func (e *EventedPLEG) sillyCacheUpdate() {
...
func (e *EventedPLEG) processCRIEvents(containerEventsResponseCh chan *runtimeapi.ContainerEventResponse) {
	for event := range containerEventsResponseCh {
		// e.updatePodStatus(event) function is blocking.
		// So handling one event at a time.
...
func (e *EventedPLEG) updatePodStatus(event *runtimeapi.ContainerEventResponse) *kubecontainer.PodStatus {
	timestamp := e.clock.Now()
	...
	if event.ContainerEventType == runtimeapi.ContainerEventType_CONTAINER_DELETED_EVENT {
		...
		// Either we use sillyCacheUpdate function above which updates the pod cache
		// timestamp periodically, or add some delay in the time stamp when we
		// set the pod cache. At this point not sure what is causing this behaviour.

		// e.cache.Set(podID, status, err, timestamp.Add(2*time.Second))
